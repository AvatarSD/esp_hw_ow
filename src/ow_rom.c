...
bool rom_is_match_family(const rom_t *rom, unsigned char family) { return (family == rom->no[0]); }

bool rom_less_then(const rom_t *rom, const rom_t *cmp) { return rom->raw < cmp->raw; }
